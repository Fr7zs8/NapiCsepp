...
BEGIN
    ...
    SELECT IFNULL(user_id, 0) INTO uid
    FROM users
...
CREATE TABLE activities (
  ...
  CONSTRAINT activities_ibfk_2 FOREIGN KEY (activity_type_id) REFERENCES types(type_id) ON DELETE CASCADE,
  CONSTRAINT activities_ibfk_3 FOREIGN KEY (activity_difficulty_id) REFERENCES difficulties(difficulty_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
...
ALTER TABLE events AUTO_INCREMENT = 56;
...
(5, 3, 'Diéta követés', 4, 3, 0, '2025-11-10', '2025-12-10', 0),
...
ALTER TABLE activities AUTO_INCREMENT = 296;
...
BEGIN
    SELECT
        ...
        (SELECT COUNT(*) FROM activities
         WHERE CURDATE() BETWEEN activity_start_date AND activity_end_date) AS total_activity_today,
        ...
        (SELECT COUNT(a.activity_id)
         ...
         WHERE LOWER(t.type_name) = 'szokás') AS total_habits;
END;
    ...
    SELECT
        (SELECT COUNT(*)
         ...
         WHERE a.user_id = p_user_id
           AND CURDATE() BETWEEN a.activity_start_date AND a.activity_end_date) AS total_activity,
 
        (SELECT COUNT(*)
         FROM activities a
         WHERE a.user_id = p_user_id
           AND a.activity_achive = 1
           AND CURDATE() BETWEEN a.activity_start_date AND a.activity_end_date) AS completed,
 
        (SELECT COUNT(*)
         FROM activities a
         WHERE a.user_id = p_user_id
           AND a.activity_type_id != 4
           AND a.activity_start_date = CURDATE()) AS daily_tasks_count,
 
        (SELECT COUNT(*)
         FROM events e
         WHERE e.user_id = p_user_id
           AND MONTH(e.event_start_time) = MONTH(CURDATE())) AS monthly_events_count,
        ...
        (SELECT COUNT(*)
         ...
         WHERE a.user_id = p_user_id
           ...
           AND a.activity_start_date = CURDATE()) AS hard_tasks,
        ...
        (SELECT COUNT(*)
         ...
         WHERE a.user_id = p_user_id
           ...
           AND a.activity_start_date = CURDATE()) AS middle_tasks,
        ...
        (SELECT COUNT(*)
         ...
         WHERE a.user_id = p_user_id
           ...
           AND a.activity_start_date = CURDATE()) AS easy_tasks,
...
CREATE PROCEDURE IF NOT EXISTS `overview`(IN p_user_id INT)
BEGIN
    SELECT
        summary.date,
...
BEGIN
    ...
    WHERE activities.user_id = p_user_id
      AND LOWER(types.type_name) = 'szokás';
END;
