...
export async function getAllActivity(req: any, res: Response) {
    ...
    try {
        const [results] = await connection.query(
            "SELECT activities.activity_id, activities.activity_type_id, activities.activity_name, types.type_name, difficulties.difficulty_name, activities.activity_achive, DATE_FORMAT(activities.activity_start_date, '%Y-%m-%d') AS activity_start_date, DATE_FORMAT(activities.activity_end_date, '%Y-%m-%d') AS activity_end_date FROM activities JOIN types on types.type_id = activities.activity_type_id JOIN difficulties ON difficulties.difficulty_id = activities.activity_difficulty_id JOIN users_activities ON users_activities.activity_id = activities.activity_id JOIN users ON users.user_id = users_activities.user_id WHERE users.user_id = ?", [id]
        ) as Array<any>
...
export async function getHabits(req: any, res: Response) {
    ...
    try {
        const [results] = await connection.query(
            "SELECT activities.activity_id, activities.activity_name, DATE_FORMAT(activities.activity_start_date, '%Y-%m-%d') AS activity_start_date, DATE_FORMAT(activities.activity_end_date, '%Y-%m-%d') AS activity_end_date, DATEDIFF(activities.activity_end_date, activities.activity_start_date) AS target_days, activities.activity_achive, users.username, types.type_name, difficulties.difficulty_name FROM activities JOIN users_activities ON activities.activity_id = users_activities.activity_id JOIN users ON users.user_id = users_activities.user_id JOIN types ON activities.activity_type_id = types.type_id JOIN difficulties ON difficulties.difficulty_id = activities.activity_difficulty_id WHERE users.user_id = ? AND types.type_name LIKE 'szokas';", [id]
        ) as Array<any>
