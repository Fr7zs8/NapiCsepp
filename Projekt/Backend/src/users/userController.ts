...
export async function signIn(req: Request, res: Response) {
  ...
  try {
    ...
    const token = jwt.sign({ user_id: results[0].id }, config.jwtSecret, {
      expiresIn: "2h",
    });
...
export async function getUser(req: any, res: Response) {
  ...
  try {
    const [results] = (await connection.query(
      "SELECT users.username, users.email, users.language, users.role, DATE_FORMAT(users.register_date, '%Y-%m-%d') AS register_date FROM users WHERE users.user_id = ?",
      [id],
    )) as Array<any>;
